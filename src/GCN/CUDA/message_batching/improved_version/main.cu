...
// Un warp elabora un messaggio; i lane dividono le feature per accessi coalescenti.
__global__ void aggregate_batch_kernel(const int* sources, const int* destinations,
    ...
    const int local_message = blockIdx.x * blockDim.y + threadIdx.y;
    ...
    for (int feature = lane; feature < feature_dim; feature += kWarpSize) {
        atomicAdd(&aggregated[destination * feature_dim + feature],
                  h_current[source * feature_dim + feature]);
    }
}

__global__ void update_kernel(const float* h_current, float* aggregated,
                              const float* weights_transposed, float* h_next,
                              const int* degree, int num_nodes, int current_dim, int next_dim) {
    const int lane = threadIdx.x;
    const int vertex = blockIdx.x * blockDim.y + threadIdx.y;
    if (vertex >= num_nodes) return;
    const int msg = vertex * current_dim;
    for (int feature = lane; feature < current_dim; feature += kWarpSize) {
    ...
    for (int output = lane; output < next_dim; output += kWarpSize) {
        ...
        for (int feature = 0; feature < current_dim; ++feature)
            dot += weights_transposed[feature * next_dim + output] * aggregated[msg + feature];
...
__global__ void softmax_kernel(float* values, int num_nodes, int num_classes) {
    ...
    const int vertex = blockIdx.x * blockDim.y + threadIdx.y;
    if (vertex >= num_nodes) return;
    const int base = vertex * num_classes;
    float local_max = -CUDART_INF_F;
    for (int c = lane; c < num_classes; c += kWarpSize)
        local_max = fmaxf(local_max, values[base + c]);
    const float maximum = warpMax(local_max);
    float local_sum = 0.0f;
    for (int c = lane; c < num_classes; c += kWarpSize) {
        const float value = expf(values[base + c] - maximum);
        values[base + c] = value;
        local_sum += value;
    }
    const float sum = warpSum(local_sum);
    for (int c = lane; c < num_classes; c += kWarpSize) values[base + c] /= sum;
}
}  // namespace
...
int main(int argc, char* argv[]) {
    ...
    if (device_count == 0) { std::cerr << "Errore: nessuna GPU CUDA rilevata.\n"; return 1; }
    int device = 0;
    ...
    std::cout << "========================================\nDataset: " << dataset_name
              ...
              << " | Classi: " << num_classes
              << "\nModalita: CUDA Pure Message-Batching Improved (1 Warp per Messaggio)"
    ...
    if (num_layers == 1) weights.emplace_back(feature_dim, num_classes);
        ...
        for (int l = 1; l < num_layers - 1; ++l) weights.emplace_back(hidden_dim, hidden_dim);
        weights.emplace_back(hidden_dim, num_classes);
    }
    std::vector<size_t> offsets(num_layers);
    size_t total_weights = 0;
    for (int l = 0; l < num_layers; ++l) { offsets[l] = total_weights; total_weights += weights[l].W.size(); }
    std::vector<float> host_weights(total_weights);
    for (int l = 0; l < num_layers; ++l) {
        const int in_dim = weights[l].in_dim, out_dim = weights[l].out_dim;
        for (int input = 0; input < in_dim; ++input)
            for (int output = 0; output < out_dim; ++output)
                host_weights[offsets[l] + static_cast<size_t>(input) * out_dim + output] =
                    weights[l].W[static_cast<size_t>(output) * in_dim + input];
    }
    ...
    const dim3 node_blocks((num_nodes + kWarpsPerBlock - 1) / kWarpsPerBlock);
    ...
    for (int l = 0; l < num_layers; ++l) {
        ...
        for (int begin = 0; begin < num_edges; begin += batch_size) {
            ...
            const dim3 blocks((count + kWarpsPerBlock - 1) / kWarpsPerBlock);
            aggregate_batch_kernel<<<blocks, threads>>>(d_sources, d_destinations, d_current,
                                                        d_aggregated, begin, count, current_dim);
    ...
    reportResults("cuda-message-batching-improved", output, graph.getLabels(), num_nodes, num_edges,
